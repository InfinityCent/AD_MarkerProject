...
# vectorization
weighted_penetrance = function(num_cells, pen) {
  wp = sum(num_cells * pen) / sum(num_cells)
...
get_wp_outliers = function(marker) {
  ...
  for (strain in strains) {
    ...
    weighted_penetrances = c(weighted_penetrances, 
                             weighted_penetrance(strain_subset$Num_cells, strain_subset$Penetrance))
  }
  ...
  pen_below_weighted = per_strain$Penetrance < per_strain$weighted_penetrance
  per_strain = cbind(per_strain, below_threshold = pen_below_weighted)
