...
## Plots cluster vs. P-value for each G20 term in ordered per_marker datasets
# -----------------------------------------------------------------------------
...
dir = 'C:/Users/peree/OneDrive/Desktop/CompBio_Code/fun_enrich_files/per_marker_ordered_custom_bg/outputs_G20'
splitAS = c('with_areashape', 'without_areashape')

terms = c("glycosylation/GPI anchor/cell wall", 
...
fun_enrich_values = function(terms, fun_enrich_df) {
  values = c()
  i = 1
  
  for (t in terms) { # for each cc term...
    if (t %in% fun_enrich_df[, 1]) { 
...
fun_enrich_table = function(directory, AS) {
  ...
  for (marker in markers) {
    ...
    for (f in all_files) {
      ...
      fun_enrich_row = c(file_id, fun_enrich_values(terms, f_df_fun_enrich))
      terms_df = rbind(terms_df, fun_enrich_row)
    }
  }
  columns = c("Cluster", terms)
  colnames(terms_df) = columns
...
write.xlsx(sheet_names, file = 'fun_enrich_per_marker_ordered_g20_custom_bg.xlsx')
